...
Official PHP client for the [Connect24](https://connect24.co.za) communications API —
one interface for email, SMS and WhatsApp.

```bash
composer require connect24/connect24
```

No package dependencies — it uses cURL, which ships with essentially every PHP install. An SDK
is a dependency of *your* application, and pulling in an HTTP client is the single most common way
a package causes a version conflict that becomes yours to resolve.
...
$client = new Client(
    ...
    apiKey: getenv('CONNECT24_API_KEY'),
);

$client->messages->sendSms('+27821234567', 'Your payment is due tomorrow.');
```

Or read both from the environment, which is what most deployments want:

```php
$client = Client::fromEnv();   // CONNECT24_ACCOUNT_ID, CONNECT24_API_KEY
```
...
```php
// SMS
$client->messages->sendSms('+27821234567', 'Your delivery is on its way.');

// WhatsApp — free-form inside the 24-hour window, a template outside it
$client->messages->sendWhatsApp('+27821234567', text: 'Your order has shipped.');
$client->messages->sendWhatsApp('+27821234567', templateName: 'payment_reminder');
...
$payload = file_get_contents('php://input');      // raw body, not $_POST
...
In Laravel, `$request->getContent()` gives you the raw body.
...
use Connect24\ConnectionException;

try {
    $client->messages->sendSms('+27821234567', 'Hello');
    ...
    print_r($e->getErrors());   // field-level validation messages, when the API sent any
...
PHP 8.1 and newer. The named arguments in the examples above need 8.0+; positional calls work
everywhere.
...
Realistic, tested sample code lives in [`samples/order-notifications`](samples/order-notifications) — an order-shipped notification
